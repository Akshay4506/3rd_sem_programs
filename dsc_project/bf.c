...
// Function to delete an element from the Bloom filter
void delete(struct BloomFilter *bf, const char *str) {
    if (!check(bf, str)) {
        printf("ERROR!\n'%s' is not present in the Bloom filter. Cannot delete.\n", str);
        return;
    }

    // Bloom filter cannot truly remove an element without risk of affecting others.
    printf("'%s' appears to be present, but Bloom filter does not support true deletion.\n", str);
}
...
void displayBloomFilter(struct BloomFilter *bf) {
    ...
    for (int i = 0; i < BLOOM_SIZE; i++) {
        ...
        if ((i + 1) % 50 == 0) {
            printf("\n"); // Newline every 50 bits for readability
        }
    }
    printf("\n");
}
